#define BMP_PG_0 0x8022B0
#define BMP_MOUNTAINS 0x87A2B0
#define BMP_TEMPLE 0x87F6B0
#define BMP_CLOUD1 0x883CB0
#define BMP_CLOUD2 0x887BB0
#define BMP_HUD 0x88C530
#define BMP_HUD_BAR 0x891330
#define BMP_HUD_KANO 0x891DB0
#define BMP_HUD_SCORPION 0x8922B0
#define BMP_FLAME 0x892BB0
#define BMP_HITBOX_BODY 0x895BB0
#define BMP_HITBOX_DUCK 0x8973B0
#define BMP_HITBOX_ATTACK 0x897FB0
#define BMPSCORPION 0x8981B0
#define BMPSCORPION_clut 0x9181B0
#define BMPKANO 0x9181D0
#define BMPKANO_clut 0x9981D0
extern void *explode_sam asm("explode_sam");
extern void *explode_sam_end asm("explode_sam_end");
extern void *fight_sam asm("fight_sam");
extern void *fight_sam_end asm("fight_sam_end");
